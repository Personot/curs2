...
class AdminChat {
    ...
    async initSocket() {
        ...
        this.socket.on('new_message', (data) => {
            console.log('AdminChat: New message', data);
        ...
        this.socket.on('chat_history', (data) => {
            console.log('AdminChat: Chat history', data);
        ...
        this.socket.on('ticket_completed', (data) => {
            console.log('AdminChat: Ticket completed', data);
            if (String(this.ticketId) === String(data.ticketId)) {
    ...
    async joinTicketRoom(ticketId) {
        console.log('joinTicketRoom called with ticketId:', ticketId);
        
        if (!ticketId) {
            console.error('joinTicketRoom: No ticketId provided');
            return;
        }
        ...
        if (this.messagesContainer) {
            this.messagesContainer.innerHTML = '<div style="text-align: center; padding: 20px; color: #94a3b8;">⏳ Загрузка истории чата...</div>';
        }

        // Убеждаемся что сокет подключен
        ...
        if (this.socket && this.socket.connected) {
            ...
            console.error('Socket not connected after init');
            if (this.messagesContainer) {
                this.messagesContainer.innerHTML = '<div style="text-align: center; padding: 20px; color: #ef4444;">❌ Ошибка подключения к чату</div>';
            }
        }
    ...
    loadChatHistory() {
        if (!this.ticketId || !this.socket || !this.socket.connected) {
            console.log('Cannot load history: no ticketId or socket not connected');
            return;
        }
        console.log('Loading chat history for ticket:', this.ticketId);
...
// Переключение заявки - ИСПРАВЛЕННАЯ ВЕРСИЯ
window.switchTicket = async (newTicketId) => {
    window.closeTicketModal();

    if (!newTicketId) {
        console.error('switchTicket: No ticketId provided');
        return;
    }

    console.log('Switching to ticket:', newTicketId);
    
    // Сохраняем в localStorage
    localStorage.setItem('lastAdminTicketId', newTicketId);
    
    // Обновляем URL без перезагрузки страницы
    const newUrl = `${window.location.pathname}?ticketId=${newTicketId}`;
    window.history.pushState({ ticketId: newTicketId }, '', newUrl);

    // Очищаем контейнер сообщений перед загрузкой новой заявки
    const messagesContainer = document.getElementById('adminChatMessages');
    if (messagesContainer) {
        messagesContainer.innerHTML = '<div style="text-align: center; padding: 20px;">⏳ Загрузка чата...</div>';
    }
...
window.initAdminChat = async (ticketId) => {
    console.log('initAdminChat called with ticketId:', ticketId);
    
    // Пытаемся получить данные админа из разных источников
    let adminData = null;
    ...
    try {
        ...
        if (response.ok) {
            const result = await response.json();
            adminData = result.user || result.data;
        }
    } catch (err) {
        console.error('Failed to fetch admin data:', err);
    }
    
    // Fallback: пробуем из localStorage/sessionStorage
    if (!adminData) {
        try {
            ...
            if (!adminData.id) {
                adminData = {
                    ...
                    full_name: sessionStorage.getItem('admin_name') || 'Администратор'
                };
            }
        } catch (err) {
            console.error('Failed to get admin data from storage:', err);
        }
    }

    const adminId = adminData?.id;
    const adminName = adminData?.full_name || adminData?.name || 'Администратор';
    ...
    if (!ticketId) {
        console.error('AdminChat: No ticketId provided');
        return null;
    }
...
window.addEventListener('popstate', async (event) => {
    ...
    if (ticketId && globalAdminChatInstance) {
        await globalAdminChatInstance.joinTicketRoom(ticketId);
    }
});
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (!ticketId) {
            ticketId = localStorage.getItem('lastAdminTicketId');
        }
        
        if (ticketId) {
            console.log('Auto-initializing with ticketId:', ticketId);
            window.initAdminChat(ticketId);
        } else {
            console.log('No ticketId found, waiting for user selection');
            // Показываем пустое состояние
            const messagesContainer = document.getElementById('adminChatMessages');
            if (messagesContainer) {
                messagesContainer.innerHTML = '<div style="text-align: center; padding: 40px; color: #94a3b8;">📭 Выберите заявку для начала работы</div>';
            }
        }
    });
} else {
