    ...
    async transferToExpert() {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Обрабатываем случай, когда нет эксперта
                if (data.error === 'No experts available' || data.error === 'expert_not_found') {
                    this.showMessageSystem('⚠️ Нет доступных экспертов для передачи заявки');
                } else {
